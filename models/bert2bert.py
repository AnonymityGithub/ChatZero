    ...
    def forward(
        ...
        **kwargs,
        ...
        if zero_setting and labels is not None:
            sample_logits = F.gumbel_softmax(decoder_outputs.logits)
            _index = torch.topk(sample_logits,k=1,dim=-1)[1]
            _index = torch.squeeze(_index,dim = -1)
            predictions_representation = self.encoder(
                input_ids=_index,
                attention_mask=None,
                inputs_embeds=None,
